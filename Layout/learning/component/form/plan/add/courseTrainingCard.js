...
const CourseTrainingCard = ({ item, removeSelected, index, length }) => {
    const { thumnnail, name, price, level, auther, language, modules, description, id, complete } = item
    return (
        <div className="relative h-full w-full flex justify-start items-center">
            <Link className="p-4 rounded-lg shadow-xl bg-white  w-full" href={`/app/learning/courses/detail/${id}`}>
                <div className="w-full">
                    ...
                    <div className="relative">
                        <button onClick={() => removeSelected(item)} className="shadow-lg absolute top-0 right-0  m-1 w-8 h-8 flex justify-center items-center text-red-400 shadow-lg bg-red-100 focus:ring-0 focus:outline-none font-medium rounded-full text-sm p-2.5 text-center inline-flex items-center">
                    ...
                    {/* <div className="flex justify-start items-center">
                        <div className="my-2 h-2 rounded-full w-full bg-gray-200">
                            <div style={{ width: `${complete + 0}%` }} className='h-2 rounded-full bg-[#409a00]'>
                            </div>
                        </div>
                    ...
                    <div className="my-3 grid grid-cols-2 gap-2">
                        <div className="flex justify-center items-center rounded-lg bg-gray-100 px-1 py-1 flex text-gray-500 font-normal text-sm">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-4 h-4 text-[#409a00]">
                                <path strokeLinecap="round" strokeLinejoin="round" d="M10.5 21l5.25-11.25L21 21m-9-3h7.5M3 5.621a48.474 48.474 0 016-.371m0 0c1.12 0 2.233.038 3.334.114M9 5.25V3m3.334 2.364C11.176 10.658 7.69 15.08 3 17.502m9.334-12.138c.896.061 1.785.147 2.666.257m-4.589 8.495a18.023 18.023 0 01-3.827-5.802" />
                            </svg>
                            <p className="ml-1">{language}</p>
                        </div>
                        <IconWithChip className='rounded-lg bg-[#409a00] text-white'>
                            Training
                        </IconWithChip>
                    </div>
                </div>
            </Link>
            {index < length - 1 ? <IconButton className='ml-2 bg-white shadow-lg text-[#419a00]'>
