...
const SessionForm = ({ batchSession, setBatchSession, index }) => {
    ...
    return (
        <div date-rangepicker='true' className="mt-4 grid grid-cols-3 gap-4">
            ...
            <div>
                <label htmlFor="startdate" className="block mb-2 text-sm font-semibold text-gray-900">Start Time</label>
                <div className="relative">
                    ...
                    <input onChange={handleValue} name="startdate" type="datetime-local" id="startdate" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full pl-10 p-2.5" placeholder="Start date start" />
                </div>
            </div>
            <div>
                <label htmlFor="enddate" className="block mb-2 text-sm font-semibold text-gray-900">End Time</label>
                <div className="relative">
                    <div className="flex absolute inset-y-0 left-0 items-center pl-3 pointer-events-none">
                        <svg aria-hidden="true" className="w-5 h-5 text-gray-500" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fillRule="evenodd" d="M6 2a1 1 0 00-1 1v1H4a2 2 0 00-2 2v10a2 2 0 002 2h12a2 2 0 002-2V6a2 2 0 00-2-2h-1V3a1 1 0 10-2 0v1H7V3a1 1 0 00-1-1zm0 5a1 1 0 000 2h8a1 1 0 100-2H6z" clipRule="evenodd"></path></svg>
                    </div>
                    <input onChange={handleValue} name="enddate" type="datetime-local" id='enddate' className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full pl-10 p-2.5" placeholder="Start date end" />
                </div>
            </div>
        </div>
    )
